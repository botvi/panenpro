                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item" aria-current="page">Form Edit Data Plan AKP</li>
                            </ul>
                        </div>
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h2 class="mb-0">Form Edit Data Plan AKP</h2>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-6">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5>Form Edit Data Plan AKP</h5>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('planakp.update', $planakp->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="mandor_id" value="{{ $planakp->mandor_id }}">
                                <div class="form-group">
                                    <label class="form-label">Blok</label>
                                    <select name="blok_id" id="blok_id" class="form-control" required>
                                        <option value="">Pilih Blok</option>
                                        @foreach ($blok as $item)
                                            <option value="{{ $item->id }}" {{ $planakp->blok_id == $item->id ? 'selected' : '' }}>{{ $item->blok }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="number" name="satuan_per_hektar" id="sph" class="form-control"
                                        placeholder="SPH (Satuan Per Hektar)" value="{{ old('satuan_per_hektar', $planakp->satuan_per_hektar) }}" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="jumlah_janjang" id="jumlah_janjang" class="form-control"
                                        placeholder="Jumlah Janjang" value="{{ old('jumlah_janjang', $planakp->jumlah_janjang) }}" required>
                                    <small>Jumlah Janjang harus berupa angka.</small>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Total (Persentase)</label>
                                    <input type="number" name="total" id="total" class="form-control" placeholder="Total" value="{{ old('total', $planakp->total) }}" required readonly>
